...
public class RegisterController {
	...
	public void registerValidate(WebDataBinder binder) {     	//WebDataBinder : 데이터의 변환, 검증기능
		...
		//스프링이 제공하는 CustomDateEditer을 이용해서 변환 String -> Date
		binder.registerCustomEditor(Date.class, new CustomDateEditor(df, false));
		binder.setValidator(new UserValidater());    //UserValidater를 Validator 로 등록
	}
	...
	public String save(@Valid User user, BindingResult result,  Model m) throws UnsupportedEncodingException {
		...
		//2. DB에 새회원 정보 저장
		if(!result.hasErrors()) {  // User 객체를 검증한 결과 에러가 있으면 registerForm를 이용해서 에러를 보여줘야 함 
