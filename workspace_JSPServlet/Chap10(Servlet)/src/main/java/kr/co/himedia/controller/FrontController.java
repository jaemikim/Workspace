...
public class FrontController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String commandStr = uri.substring(lastSlash);		// 인덱스로 경롱의 마미작 부분의 문자열을 얻어옴
		...
		request.setAttribute("uri", uri);					// request 영역에 저
		request.setAttribute("commandStr", commandStr);		//
		request.getRequestDispatcher("/servlet/frontController.jsp").forward(request, response);	// frontController.jsp로 포원드
