<%@page import="kr.co.himedia.jst.PersonDTO"%>
<%@page import="java.util.Date"%>
<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%@ taglib prefix="c" uri="jakarta.tags.core" %> <!-- JSTL 사용시 태그 앞에 붙이 접두어, 태그 라이브러리 URI 식별자 -->    
<!DOCTYPE html>
<html>
<head>
  <meta charset="UTF-8">
...
<body>
	...
	<!-- 자바빈즈 생성 시 생성자를 통해 초기값 설정함, request 영역에 변수를 저장 -->
	<h4>자바빈즈 생성 - 생성자 사용</h4>
	<c:set var="personVar" value='<%= new PersonDTO("이순신", 43) %>' scope="request"/>
	<ul>
		<li>이름 : ${requestScope.personVar.name }</li>
		<li>나이 : ${presonVar.age }</li>
	</ul>
	
	<!--  자바빈즈를 생성 후 값을 나중에 설정하려면 target, property 속성을 사용함, target에는 변수명 지정하고, property에는 멤버 변수명을 지정 -->
	...
	<ul>
		...
		<li>나이 : ${requestScope.personVar2.age }</ii>
	</ul>
</body>
</html>