...
<body>
	<h3>변수 선언 및 할당</h3>
	스크립틀릿에서 선언한 변수 : ${num1 } <br/>    <!-- EL은 스클립틀릿 선언한 변수를 즉시 사용할수 없음 -->
	page 영역에 저장된 병수 : ${num2 } <br/>		<!-- page 영역에 저장된 속성은 정상적으로 출력됨 -->
	변수 할당 및 즉시 출력 : ${num1 = 7 } <br/>
	별수 할당 및 별도 출력 : ${num2 = 8; ''} = ${num2 } <br/>
	num1 = ${mun1 }, num2 = ${mun2 }, num3 = ${mun3 }, num4 = ${mun4 }
	...
	<h3>+ 연산자는 덧셈만 가능</h3>	
	num1 + "34" : ${num1 + "34" }<br/>
	<%-- num2 + "이십" : ${num2 + "이십"} <br/> --%> <!-- 숫자 형태의 문자열이라면 자동으로 숫자로 변횐되어 계산됨, 문자가 포함되어 있다면 반환 안됨 -->
	<%--  "삼십" + "사십" : ${"삼십" + "사십"} --%>
