...
@WebServlet("/mvcwebboard/pass.do")
public class PassController extends HttpServlet {

	@Override
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		request.setAttribute("mode", request.getParameter("mode"));
		request.getRequestDispatcher("/mvcWebBoard/pass.jsp").forward(request, response);
	}
...
//		System.out.println("doPost 요청");
		...
		if (confirmed) {
			if(mode.equals("edit")) {             //수정모드
				...
				response.sendRedirect("../mvcwebboard/edit.do?id=" + id);
				...
				JSFunction.alertLocation(response, "삭제되었습니다.", "../mvcwebboard/list.do");
			}
			
		} else {   								  // 비밀번호 불일치
			JSFunction.alertBack(response, "비밀번호가 검증에 실패했습니다. ");
		}
	}
}
