...
@WebServlet("/mvcwebboard/edit.do")
@MultipartConfig(
...
//		System.out.println("doGet 호출");
		...
		request.getRequestDispatcher("/mvcWebBoard/edit.jsp").forward(request, response);
	}
...
//		System.out.println("doPost 호출");
		...
		try {
			...
			JSFunction.alertBack(response, "파일 업르도 오류입니다");
		}
...
//		System.out.println("id = " +id);
		...
		if (result == 1) {
			...
			response.sendRedirect("../mvcwebboard/view.do?id=" + id);
		} else {  // 수정 실패
			JSFunction.alertLocation(response, "비밀번호 검증을 다시 진행해주세요", "../mvcwebboard/view.do?id=" + id);
		}
	}
}
