...
-- LIST_PRICE가 650 이상이면서 680이하인 행(들)을 출력하시오
SELECT P.PRODUCT_NAME 
	, P.LIST_PRICE 
FROM  PRODUCTS p 
...
-- CATEGORY_ID가 1혹은 4일 행(들)을 출력하시오.
SELECT P.PRODUCT_NAME 
, P.CATEGORY_ID  
FROM PRODUCTS p 
...
-- 데이터 삽입 (CURRENT_DATE 오늘 날짜가 들어간다.)
INSERT INTO 
	DISCOUNTS A (
					A.DISCOUNT_NAME 
					, A.AMOUNT 
...
-- 테이블 생성 NUMBER(2, 0) 뒤에 소수점자리를 나타내는 것 0이면 소수점 자리는 없는거다. 
-- PRIMARY KEY (PART_ID) - PK 이름을 지정하지 않아서 자동으로 생성된다.
CREATE TABLE PARTS
(
...
-- 전체 행을 업데이트
-- WHERE 절이 벗는 UPDATE문은 매우 위험한 작업이므로 경고창 뜸
UPDATE PARTS 
	SET COST = COST * 1.05
;	

-- 두개의 테이블로 하나의 테이블을 생성함
CREATE TABLE SALES
AS
SELECT
		A.ORDER_ID 
