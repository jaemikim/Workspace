...
-- 사원들 중 부서 코드 별 가장 늦은 생일, 가장 빠른 생일, 직원 수 
...
-- SELECT 작성한 쿼리를 스칼라 서브쿼리라고 한다. (SELECT B.DEPT_NM FROM TB_DEPT B WHERE B.DEPT_CD = A.DEPT_CD) AS DEPT_NM
SELECT A.DEPT_CD 
 ...
 * 죽 부서의 직원수가 1명을 초과하는 부서의 집계 데이터를 출력하시오
 */
SELECT A.DEPT_CD 
	   , (SELECT B.DEPT_NM FROM TB_DEPT B WHERE B.DEPT_CD = A.DEPT_CD) AS DEPT_NM
	   , MAX(A.BIRTH_DE) AS "가장 늦은 생년월일"
